...
public class Knight extends Entity {
    ...
    public Knight(boolean isEnemy, float x, float y, int width, int height, int health, int price, int range, int damage, long recoilTime, GamePanel panel) {
        ...
        if (isEnemy) {
            ...
            this.sprite = KnightResources.walkFramesEnemy[0];
        } else {
            walkFrames = KnightResources.walkFramesPlayer;
            attackFrames = KnightResources.attackFramesPlayer;
            this.sprite = KnightResources.walkFramesPlayer[0];
        }
    }
